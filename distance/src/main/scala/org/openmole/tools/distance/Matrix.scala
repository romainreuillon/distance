...
// private def toIndex (coor :Int*): Int = (for ((arg,i) <- coor.init.reverse zip (1 until coor.length-1)) yield arg* dim.slice(0, i).product).sum + coor.last
 ...
 def init : Unit = for(i<- 0 until mat.length) {mat(i)= new BorderPoint}
 ...
 class MatIterator () {
   ...
   //private val matrice : Matrix = m
    ...
    def setLastAxis (axis: Int)={
                                  require(axisIsOk(axis))
                                  if(axis==0) {index+=dim.last-getAxis(0)-1}
    ...
    def isAxisLast(axis:Int) ={ require(axisIsOk(axis))
                               getAxis(axis)==dim.reverse(axis)-1
                              }
    def isAxisFirst(axis:Int) ={ require(axisIsOk(axis))
                               getAxis(axis)==0
    }
